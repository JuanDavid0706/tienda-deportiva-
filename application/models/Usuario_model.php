...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Usuario_model extends CI_Model{
		...
		function agregar_productos($productos) {
			$resultado = $this->db2->insertOne($productos);
			if($resultado->getInsertedCount()>0){
				return array ("guardado"=> true, "productos_id"=>$resultado->getInsertedId());
			}
			else{
		...
		function agregar_persona($persona) {
			$resultado = $this->db->insertOne($persona);
			if($resultado->getInsertedCount()>0){
				return array ("guardado"=> true, "pedidos_id"=>$resultado->getInsertedId());
			}
			else{
					return array ("guardado"=>false);
